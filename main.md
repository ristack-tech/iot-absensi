...
// --- Konfigurasi ---
#define WIFI_SSID     "alamak"
#define WIFI_PASS     "harusbisa"
#define BACKEND_BASE  "http://192.168.1.81:8000/api/device"
#define DEVICE_TOKEN  "alat-gudang1-secret"

// --- Timing ---
#define SCAN_COOLDOWN_MS  3000
#define WIFI_RETRY_MS     30000
#define POLL_INTERVAL_MS  60000  // fix: dari 5000 → 60 detik
...
void sendTaskResult(int taskId, const char* status, const char* errorMsg = nullptr);
...
void setup() {
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, OLED_ADDR)) {
    ...
    oledOk = false;
    // Lanjut tanpa OLED — AS608 & WiFi tetap jalan
  } else {
    oledOk = true;
    display.clearDisplay();
    display.display(); // flush noise setelah begin
    delay(50);
  ...
  WiFi.mode(WIFI_STA);
  WiFi.setSleep(false); // fix: matikan modem sleep → koneksi lebih stabil
...
void sendAttendance(int fingerprintId) {
  ...
  if (httpCode == 200) {
    ...
    if (!err) {
      ...
      // fix: cari separator " · " (U+00B7) sesuai format server, fallback ke " - "
      int d = oledText.indexOf(" \xC2\xB7 ");
      if (d < 0) d = oledText.indexOf(" - ");
      if (d >= 0) {
        l1 = oledText.substring(0, d).substring(0, 21);
        String rest = oledText.substring(d + 3);
        int d2 = rest.indexOf(" \xC2\xB7 ");
        if (d2 < 0) d2 = rest.indexOf(" - ");
        if (d2 >= 0) {
...
// =============================================================================
// HELPER: Kirim result task ke server
// =============================================================================
void sendTaskResult(int taskId, const char* status, const char* errorMsg) {
  StaticJsonDocument<128> res;
  res["status"]        = status;
  res["error_message"] = errorMsg; // null kalau tidak diisi
  ...
  HTTPClient hRes;
  hRes.begin(String(BACKEND_BASE) + "/tasks/" + String(taskId) + "/result");
...
// =============================================================================
// TASK POLLING — cek tugas dari server setiap 60 detik
// =============================================================================
void tickTaskPoller() {
  ...
  for (JsonObject task : tasks) {
    ...
    if (action == "delete_template") {
      ...
      sendTaskResult(id, "done");
      showOLED("Hapus OK!", "Slot #" + String(fpId), "");
      delay(1000);

    } else if (action == "enroll_capture" || action == "sync_template") {
      // fix: kirim failed agar task tidak looping terus di server
      sendTaskResult(id, "failed", "not implemented");
      Serial.printf("[TASK] %s — not implemented, replied failed\n", action.c_str());
    }
  }
...
// =============================================================================
// OLED DISPLAY
// PENTING: selalu clearDisplay() + tulis + display() — tidak ada delay() di sini
// =============================================================================
void showOLED(const String& l1, const String& l2, const String& l3) {
  if (!oledOk) {
  ...
  if (l1.length() > 0) { display.setCursor(0,  0); display.println(l1.substring(0, 21)); }
  if (l2.length() > 0) { display.setCursor(0, 22); display.println(l2.substring(0, 21)); }
  if (l3.length() > 0) { display.setCursor(0, 44); display.println(l3.substring(0, 21)); }
